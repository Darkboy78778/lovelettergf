...
const GiftPage = () => {
  ...
  const themeEmoji = { love: '❤️', birthday: '🎂', friendship: '🤝', romantic: '🌹', surprise: '🎉' };

  return (
    <div className="min-h-screen gradient-romantic relative overflow-hidden">
      <AnimatePresence mode="wait">
        {stage === 'intro' && (
          <motion.div key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.1 }} transition={{ duration: 0.5 }} className="min-h-screen flex flex-col items-center justify-center px-6">
            <FloatingHearts count={12} />
            <SparkleParticles count={20} />
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 150, delay: 0.3 }} className="text-8xl mb-8">
              {gift.theme === 'birthday' ? '🎁' : '💌'}
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="font-display text-2xl md:text-3xl font-bold text-center mb-3">
              A letter for you
            </motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="text-muted-foreground font-body text-lg mb-8">
              from <span className="text-primary font-semibold">{gift.sender_name}</span>
            </motion.p>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }}>
              <Button size="lg" onClick={() => { setStage('opening'); setTimeout(() => setStage('revealed'), 2000); }} className="rounded-full px-10 text-lg font-body gap-2 animate-float">
        ...
        {stage === 'opening' && (
          <motion.div key="opening" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen flex items-center justify-center">
            <SparkleParticles count={30} />
            <motion.div initial={{ rotateY: 0 }} animate={{ rotateY: 180, scale: [1, 1.2, 0.8] }} transition={{ duration: 1.5, ease: 'easeInOut' }} className="text-9xl">
              {gift.theme === 'birthday' ? '🎁' : '💌'}
            </motion.div>
          </motion.div>
        )}
        ...
        {stage === 'revealed' && (
          <motion.div key="revealed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className="min-h-screen py-12 px-4">
            <FloatingHearts count={8} />
            <SparkleParticles count={15} />
            <div className="relative z-10 max-w-lg mx-auto">
              <motion.div initial={{ opacity: 0, y: 40, rotateX: 20 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="glass-card rounded-2xl p-8 md:p-10 mb-8">
                <div className="text-center mb-6"><span className="text-3xl">{themeEmoji[gift.theme]}</span></div>
                <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="font-display text-2xl font-bold text-center mb-2">
                  Dear {gift.recipient_name},
                </motion.h2>
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="font-body text-foreground/90 leading-relaxed whitespace-pre-wrap text-center mt-6">
                  {gift.message}
                </motion.div>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="font-display text-lg text-right mt-8 text-primary italic">
              ...
              {gift.photos.length > 0 && (
                <div className="space-y-4">
                  {gift.photos.map((photo, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 30, rotate: i % 2 === 0 ? -3 : 3 }} animate={{ opacity: 1, y: 0, rotate: i % 2 === 0 ? -2 : 2 }} transition={{ delay: 1.4 + i * 0.3, duration: 0.6 }} className="glass-card rounded-xl p-3 shadow-lg">
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }} className="text-center text-sm text-muted-foreground font-body mt-8">
