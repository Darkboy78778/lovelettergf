...
const FloatingMessages = ({ theme, senderName, recipientName, specialDate, onComplete }: FloatingMessagesProps) => {
  ...
  const generateMessages = useCallback(() => {
    ...
    for (let i = 0; i < count; i++) {
      // Spread evenly: 0→100% across screen width
      const evenX = ((i + 0.5) / count) * 100;
      // Add small random jitter (±3%)
      const jitter = (Math.random() - 0.5) * 6;
      generated.push({
        id: i,
        text: allMessages[i % allMessages.length],
        x: Math.max(5, Math.min(95, evenX + jitter)),
  ...
  return (
    <div className="min-h-screen relative overflow-hidden gradient-romantic">
      ...
      {messages.map((msg) => (
        <motion.div
          key={msg.id}
          className="absolute font-display font-semibold text-primary whitespace-nowrap pointer-events-none will-change-transform -translate-x-1/2"
          style={{
            left: `${msg.x}%`,
